...
// Компоновка элементов панели "Сообщение"
        ...
        layout2.setVerticalGroup(layout2.createSequentialGroup()
                        .addContainerGap() .addGroup(layout2.createParallelGroup()
...
// Получаем необходимые параметры
            ...
            for(int i=0;i<destinationAddress.length();i++) {
                for (int j = 0; j <symbol.length; j++) {
                    if (destinationAddress.charAt(i) == symbol[j].charAt(0)) {
                        k++;
                        j=22;
                    }
                }
            }
            if(k!=destinationAddress.length()){
                error=true;
            }

            String[] gl=destinationAddress.split("\\.");
// Убеждаемся, что поля не пустые

            if(!error) {
                for (String retval : gl) {
                    k = Integer.parseInt(retval);
                    if (k > 256 || k < 0) {
                        error = true;
                    }
                }
